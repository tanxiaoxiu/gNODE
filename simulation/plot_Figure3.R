...
###############setting
base_path <- "~/gNODE/simulation/result2/"
...
for (P in P_values) {
  ...
  ###########mean_rrmse_iter_mean
  all_rrmse_mean_iter_mean <- all_results$all_rrmse_mean_iter_mean
  all_rrmse_mean_iter_mean <- na.omit(all_rrmse_mean_iter_mean)
  all_rrmse_mean_iter_mean$Method <- factor(all_rrmse_mean_iter_mean$Method, levels = c("NODE", "pNODE", "gNODE"))
  all_rrmse_mean_iter_mean$SubjectN <- factor(all_rrmse_mean_iter_mean$SubjectN)
  all_rrmse_mean_iter_mean$Timepoint <- as.factor(all_rrmse_mean_iter_mean$Timepoint)
  ...
  plot_all_rrmse_mean_iter_mean <- ggplot(all_rrmse_mean_iter_mean, aes(x = Timepoint, y = rrmse_mean_iter_mean, fill = Method)) +
  ...
  plot_all_rrmse_mean_iter_mean
  ggsave(filename="Figure3.png",plot=plot_all_rrmse_mean_iter_mean,device="png",dpi=600,units="in",width=18,height=20)
