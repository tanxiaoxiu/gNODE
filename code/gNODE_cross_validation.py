...
class ODEFunc(torch.nn.Module):
    def __init__(self, p_dim):
        super().__init__()
        self.p_dim = p_dim
        self.alpha = torch.nn.Parameter(torch.rand(p_dim, device=device))
        self.beta = torch.nn.Parameter(self._generate_A(p_dim))
    ...
    def _generate_A(self, p_dim):
        a_data = torch.normal(mean=0.0, std=0.1, size=(p_dim, p_dim), device=device)
...
def train_and_evaluate(train_data, val_data, run_device, batch_size, output_dir, fold_idx, timepoints, p_dim):
    ...
    while not successful_train and current_seed <= max_seed_attempts:
        try:
            ...
            model = ODEFunc(p_dim).to(run_device)
            ...
            for epoch in range(num_epochs):
                ...
                for input_data, output_data in train_loader:
                    input_data, output_data = input_data.to(run_device), output_data.to(run_device)
                    optimizer.zero_grad()
            ...
            print(f"Fold {fold_idx} trained successfully with seed {current_seed}.")
        except AssertionError as e:
            if "underflow in dt" in str(e):
                print(
                    f"Underflow error on fold {fold_idx} with seed {current_seed}. "
    ...
    if not successful_train:
        print(f"Failed to train after trying {max_seed_attempts} seeds for fold {fold_idx}.")
...
def main():
    ...
    for iter_num in range(args.iter_start, args.iter_end + 1):
        ...
        avg_rmse = k_fold_cross_validation(
            tensor_data, num_folds, device, batch_size, output_dir_path, timepoints, p_dim
        )

        print(
            f"sparsity={args.sparsity_label}, subject={args.s_label}, "
            f"time={args.t_label}, iter={iter_num}, Average Relative RMSE: {avg_rmse:.4f}"
        )
